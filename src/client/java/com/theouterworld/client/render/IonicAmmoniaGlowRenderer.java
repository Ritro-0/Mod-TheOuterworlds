...
/**
 * Glow overlay for ionic ammonia deposits.
 */
public final class IonicAmmoniaGlowRenderer {
	private static final Identifier GLOW_TEXTURE = OuterWorldMod.id("textures/block/ammonia_still.png");
	...
	private IonicAmmoniaGlowRenderer() {
	}

	public static void register() {
		LevelExtractionEvents.END_EXTRACTION.register(IonicAmmoniaGlowRenderer::extract);
		LevelRenderEvents.COLLECT_SUBMITS.register(IonicAmmoniaGlowRenderer::submit);
	}
	...
	private static void collectFromChunk(LevelChunk chunk, List<BlockPos> found) {
		...
		for (int i = 0; i < sections.length; i++) {
			...
			if (!section.maybeHas(state -> state.is(ModBlocks.IONIC_AMMONIA))) {
				continue;
			}
			...
			for (int y = 0; y < 16; y++) {
				for (int z = 0; z < 16; z++) {
					for (int x = 0; x < 16; x++) {
						...
						if (state.is(ModBlocks.IONIC_AMMONIA)) {
