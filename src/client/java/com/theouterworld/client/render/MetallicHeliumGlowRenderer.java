...
/**
 * Glow-squid / ethereal-orb style eyes overlay for metallic helium sources.
 */
public final class MetallicHeliumGlowRenderer {
	private static final Identifier GLOW_TEXTURE = OuterWorldMod.id("textures/block/helium_still.png");
	...
	private MetallicHeliumGlowRenderer() {
	}

	public static void register() {
		LevelExtractionEvents.END_EXTRACTION.register(MetallicHeliumGlowRenderer::extract);
		LevelRenderEvents.COLLECT_SUBMITS.register(MetallicHeliumGlowRenderer::submit);
	}
	...
	private static void collectFromChunk(LevelChunk chunk, List<BlockPos> found) {
		...
		for (int i = 0; i < sections.length; i++) {
			...
			if (!section.maybeHas(state -> state.is(ModBlocks.METALLIC_HELIUM))) {
				continue;
			}
			...
			for (int y = 0; y < 16; y++) {
				for (int z = 0; z < 16; z++) {
					for (int x = 0; x < 16; x++) {
						...
						if (state.is(ModBlocks.METALLIC_HELIUM)) {
