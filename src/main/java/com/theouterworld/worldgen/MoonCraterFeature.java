...
public class MoonCraterFeature implements Feature {
	...
	public boolean place(WorldGenLevel world, ChunkGenerator chunkGenerator, RandomSource random, BlockPos origin) {
		...
		return applyGrid(world, chunk, world.getSeed() + 7919, minX, minZ, maxX, maxZ);
	}
	...
	private static boolean applyCellCrater(
		...
		for (int x = x0; x <= x1; x++) {
			for (int z = z0; z <= z1; z++) {
				...
				for (int y = surfaceY - 1; y >= surfaceY - 12 && y > minY; y--) {
					cursor.set(x, y, z);
					BlockState below = world.getBlockState(cursor);
					if (below.is(Blocks.BEDROCK) || below.is(ModBlocks.GABBRO) || below.is(ModBlocks.PYROXENITE) || below.is(ModBlocks.NORITE)) {
						break;
					}
