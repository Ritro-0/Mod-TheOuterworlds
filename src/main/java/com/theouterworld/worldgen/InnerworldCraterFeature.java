...
/**
 * Large, evenly spaced impact craters for the Innerworld (lunar highlands).
...
public class InnerworldCraterFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		int maxX = minX + 15;
		int maxZ = minZ + 15;
		return applyGrid(world, world.getSeed() + 7919, minX, minZ, maxX, maxZ);
	}
	...
	private static boolean applyGrid(WorldGenLevel world, long seed, int minX, int minZ, int maxX, int maxZ) {
		...
		for (int cellX = cellMinX; cellX <= cellMaxX; cellX++) {
			for (int cellZ = cellMinZ; cellZ <= cellMaxZ; cellZ++) {
				if (applyCellCrater(world, seed, cellX, cellZ, minX, minZ, maxX, maxZ)) {
					placed = true;
				}
			}
		}
	...
	private static boolean applyCellCrater(
		...
		int radius;
		if (sizeRoll < 0.55) {
			radius = 22 + (int) (WorldgenNoise.hash(seed + 67, cellX, cellZ) * 5.0);
		} else if (sizeRoll < 0.90) {
			radius = 27 + (int) (WorldgenNoise.hash(seed + 67, cellX, cellZ) * 6.0);
		} else {
			radius = 33 + (int) (WorldgenNoise.hash(seed + 67, cellX, cellZ) * 4.0);
		}
		...
		double depth = radius * (0.32 + 0.08 * WorldgenNoise.hash(seed + 149, cellX, cellZ));
		double rimWidth = 3.2 + radius / 6.0 + 2.0 * WorldgenNoise.hash(seed + 331, cellX, cellZ);
		double rimHeight = 2.6 + radius / 6.5 + 1.6 * WorldgenNoise.hash(seed + 347, cellX, cellZ);
		boolean complex = radius >= 28;
		double peakHeight = complex ? radius / 7.5 : 0.0;
		...
		BlockState norite = ModBlocks.NORITE.defaultBlockState();
			...
			for (int z = z0; z <= z1; z++) {
				...
				if (surfaceY > originalSurface) {
					for (int y = originalSurface + 1; y < surfaceY; y++) {
						cursor.set(x, y, z);
						if (!world.getBlockState(cursor).is(Blocks.BEDROCK)) {
							world.setBlock(cursor, norite, 2);
						}
					}
				}
				...
				BlockState surface;
				boolean exposeBedrock = inner && t < 0.50 && !(complex && t < 0.20);
				if (exposeBedrock) {
					surface = exposedStone(existing, norite);
				} else {
					surface = regolith;
				}
				...
				if (inner && t < 0.70) {
					for (int y = surfaceY - 1; y >= surfaceY - 2 && y > minY; y--) {
						...
						if (below.is(Blocks.BEDROCK) || below.is(ModBlocks.GABBRO) || below.is(ModBlocks.PYROXENITE)) {
							continue;
						}
						if (below.is(ModBlocks.ANORTHOSITIC_REGOLITH) || below.isAir()) {
							world.setBlock(cursor, norite, 2);
						}
					}
				}
	...
	private static BlockState exposedStone(BlockState existing, BlockState norite) {
		if (existing.is(ModBlocks.GABBRO) || existing.is(ModBlocks.PYROXENITE) || existing.is(ModBlocks.NORITE)) {
			return existing;
		}
		return norite;
	}
