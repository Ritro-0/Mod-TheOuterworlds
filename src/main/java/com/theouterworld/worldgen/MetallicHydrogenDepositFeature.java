...
public class MetallicHydrogenDepositFeature extends Feature<NoneFeatureConfiguration> {
	...
	public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> context) {
		...
		int y = HighworldLayers.METAL_TOP_Y + MIN_ABOVE_METAL + random.nextInt(MAX_ABOVE_METAL - MIN_ABOVE_METAL + 1);
		cursor.set(x, y, z);
		if (!isLiquidHydrogen(world.getFluidState(cursor))) {
			return false;
		}
	...
	private static boolean placeBlob(WorldGenLevel world, BlockPos center, int size, RandomSource random) {
		BlockState metallic = ModBlocks.METALLIC_HYDROGEN.defaultBlockState();
		BlockPos.MutableBlockPos cursor = new BlockPos.MutableBlockPos();
		int placed = 0;
		...
		for (int i = 0; i < size * 6 && placed < size; i++) {
			...
			if (cursor.getY() < HighworldLayers.METAL_TOP_Y + MIN_ABOVE_METAL
				|| cursor.getY() > HighworldLayers.METAL_TOP_Y + MAX_ABOVE_METAL) {
				continue;
			}
			if (!isLiquidHydrogen(world.getFluidState(cursor))) {
				continue;
		...
		if (placed == 0 && isLiquidHydrogen(world.getFluidState(center))) {
	...
	private static boolean isLiquidHydrogen(FluidState fluid) {
		return fluid.is(ModFluids.LIQUID_HYDROGEN) || fluid.is(ModFluids.FLOWING_LIQUID_HYDROGEN);
	}
}
