...
/**
 * Liquid ammonia: same swim / burn profile as liquid hydrogen.
 */
public final class LiquidAmmoniaFluidInteractions {
	private static final double ASCEND_VELOCITY_SCALE = 0.22;
	private static final double ASCEND_VELOCITY_CAP = 0.035;

	private LiquidAmmoniaFluidInteractions() {
	}
	...
	public static void register() {
		...
		EntityFluidInteractionRegistry.register(ModTags.LIQUID_AMMONIA, new FluidBehavior() {
			@Override
	...
	private static void burnEntity(Entity entity) {
		...
		if (entity instanceof LivingEntity living
			&& IridiumProtection.countIridiumPieces(living) >= FarworldLayers.FARWORLD_IRIDIUM_PIECES) {
			entity.clearFire();
			return;
		}
